...
import apiClientSource from "../../src/api.ts?raw";
...
describe("API-Client", () => {
  ...
  it("stellt keine stillgelegten Finanzmethoden oder -pfade mehr bereit", () => {
    expect(apiClientSource).not.toMatch(/finance/i);
    expect(Object.keys(api)).not.toContain("getFinance");
    expect(Object.keys(api)).not.toContain("exportFinance");
  });
});
